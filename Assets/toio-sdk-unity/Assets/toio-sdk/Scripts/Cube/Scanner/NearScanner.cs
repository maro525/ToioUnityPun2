    ...
    public class NearScanner : NearScannerInterface
    {
        ...
        public class Impl : NearScannerInterface
        {
            ...
            public UniTask<BLEPeripheralInterface[]> Scan(float waitSeconds = 3.0f)
            {
                ...
                var periArray = Array.ConvertAll<GameObject, UnityPeripheral>(GameObject.FindGameObjectsWithTag("Cube"), obj => new UnityPeripheral(obj));
                foreach (var peri in periArray)
                {
                    if (this.peripheralList.Count < this.satisfiedNum)
                    {
            ...
            public void ScanAsync(MonoBehaviour coroutineObject, Action<BLEPeripheralInterface> callback, bool autoRunning)
            {
                ...
                var peris = Array.ConvertAll<GameObject, UnityPeripheral>(GameObject.FindGameObjectsWithTag("Cube"), obj => new UnityPeripheral(obj));
