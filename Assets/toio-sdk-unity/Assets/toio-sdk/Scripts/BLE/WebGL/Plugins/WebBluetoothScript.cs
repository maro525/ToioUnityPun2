...
namespace toio
{
    public class WebBluetoothScript : MonoBehaviour
    {
        ...
        {
#if UNITY_WEBGL
            if (this.callback_RequestDevice != null)
            {
                Debug.LogError("[WebBluetoothScript.RequestDevice]複数回呼ぶ事は出来ません");
                return;
            }
        ...
        {
#if UNITY_WEBGL
            if (this.callbackTable_Connect.ContainsKey(deviceID))
            {
                Debug.LogError("[WebBluetoothScript.Connect]複数回呼ぶ事は出来ません");
                return;
            }

            this.callbackTable_Connect.Add(deviceID, callback);
            if (null != disconnectedCallback)
            {
                callbackTable_Disconnected.Add(deviceID, disconnectedCallback);
            }
        ...
        public void GetCharacteristic(int serviceID, string characteristicUUID, Action<int, string> callback)
        {
            ...
            if (this.callbackTable_GetCharacteristic.ContainsKey(str))
            {
                Debug.LogError("[WebBluetoothScript.GetCharacteristic]複数回呼ぶ事は出来ません");
                return;
            }

            this.callbackTable_GetCharacteristic.Add(str, callback);
        ...
        {
#if UNITY_WEBGL
            if (this.callbackTable_GetCharacteristics.ContainsKey(serviceID))
            {
                Debug.LogError("[WebBluetoothScript.GetCharacteristics]複数回呼ぶ事は出来ません");
                return;
            }
            this.callbackTable_GetCharacteristics.Add(serviceID, callback);
        ...
        {
#if UNITY_WEBGL
            if (this.callbackTable_ReadValue.ContainsKey(characteristicID))
            {
                Debug.LogError("[WebBluetoothScript.ReadValue]複数回呼ぶ事は出来ません");
                return;
            }

            this.callbackTable_ReadValue.Add(characteristicID, callback);
        ...
        {
#if UNITY_WEBGL
            if (this.callbackTable_StartNotifications.ContainsKey(characteristicID))
            {
                Debug.LogError("[WebBluetoothScript.StartNotifications]複数回呼ぶ事は出来ません");
                return;
            }

            this.callbackTable_StartNotifications.Add(characteristicID, callback);
        ...
        public void StopNotifications(int characteristicID)
        {
            ...
            if (!this.callbackTable_StartNotifications.ContainsKey(characteristicID))
            {
                Debug.LogError("[WebBluetoothScript.StopNotifications]複数回呼ぶ事は出来ません");
                return;
            }
